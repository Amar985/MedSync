...
def train_tft(data_path):
    ...
    df.drop(
        columns=["Cost", "Total_product_purchase", "PRODUCTO", "PURCHASING_REGION"],
        inplace=True,
    )

    df["P_code_NUM"] = df["P_code_NUM"].astype(str)
    ...
    training = TimeSeriesDataSet(
        train,
        time_idx="time_idx",
        target="N_product_purchased",
        group_ids=["P_code_NUM", "PURCHASING_DEPARTMENT", "PURCHASING_HOSPITAL"],
        ...
        time_varying_unknown_reals=["N_product_purchased"],
        categorical_encoders={"Type_purchase": NaNLabelEncoder(add_nan=True)},
        # lags={'N_product_purchased': [30, 90, 365]},
    ...
    joblib.dump(tft, 'tft_model.pkl')  # or pickle.dump(tft, open('tft_model.pkl', 'wb'))
