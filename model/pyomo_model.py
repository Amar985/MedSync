...
class PartialModel(pyo.ConcreteModel):
    ...
    def compute_xi(self) -> list:
        ...
        Returns:
            xi (list): demand for each month
        """
        data_xi = self.data.groupby(["MONTH"]).N_product_purchased.sum().reset_index()
        data_xi = data_xi.set_index("MONTH")
        return [
            data_xi.loc[i, "N_product_purchased"] if i in data_xi.index else 0
            for i in range(1, 13)
        ]
    ...
    def process_data(data: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        data = data.copy()
        data["P_date"] = pd.to_datetime(data["P_date"], dayfirst=True)

        # Select useful columns
        data["YEAR"] = data["P_date"].dt.year
        data["MONTH"] = data["P_date"].dt.month
        data = data[["YEAR", "MONTH", "N_product_purchased"]]
